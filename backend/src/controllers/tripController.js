  ...
  try {
    ...
    if (overlapCheck.rows.length > 0) {
      ...
      return res.status(400).json({
        message: `Trip overlaps with existing trip (${t.destination}) from ${t.start_date.toISOString().slice(0,10)} to ${t.end_date.toISOString().slice(0,10)}.`,
      });
    }
    ...
    const result = await pool.query(
      ...
      `,
      [user_id, title ?? null, destination, start_date, end_date, budget ?? null]
    );
...
export const updateTrip = async (req, res) => {
  try {
    ...
    // 1) Load the current trip (to coalesce missing fields)
    const curr = await pool.query(
      `SELECT id, user_id, title, destination, start_date, end_date, budget
    ...
    // 2) Coalesce incoming with current
    ...
    if (!newDestination || !newStart || !newEnd) {
      return res.status(400).json({ message: "All fields required" });
    }
